/**
 * Shared Contract Layer (Frontend)
...
function assertValidStatus(status) {
  if (!VALID_STATUS.has(status)) {
    throw new ValidationError('Invalid chapter status', { status });
  }
}
 ...
 * Frontend usage:
 * 
 * import { validators, enforce, CONTRACT_VERSION } from '../contracts';
 * 
 * async function loadSession(id) {
 *   const res = await fetch(`/api/stream/session/${id}`);
 *   const data = await res.json();
 *   
 *   validators.validateSession(data.session); // throws if invalid
 *   validators.validateScene(data.scenes[0]); // throws if invalid
 *   
 *   return data;
 * }
 * 
 * // Include version in requests
 * const response = await api.post('/api/stream/story/sync', {
 *   ...payload,
